...
## FFD Segmentation of Macroeconomic Release Series

This document describes a protected public version of the pipeline used to test whether the memory structure of a macroeconomic release series changes across an exogenous regime window.

## Notation

- x_t: raw macro release series
- x_t_tilde: transformed series; log for index-like variables and levels for rate-like variables
- d: fractional differencing order on an admissible grid from 0 to 1
- FFD_d(x_t_tilde): fixed-width fractionally differenced series
- d_star: minimal admissible differencing order under the ADF criterion
- d_global: minimal admissible order estimated on the baseline sample
- d_local: minimal admissible order estimated on the regime-training sample
- Delta_d: d_local minus d_global
...
For rate-like series, the transformation is the identity, so the series is kept in levels.
...
This truncation determines the effective width of the fixed-width filter.

### 3. Minimal admissible order

For each candidate order on a coarse admissible grid from 0 to 1:
...
For a given series and regime window:
...
4. estimate d_global on the baseline sample,
5. estimate d_local on the regime-training partition,
6. score both orders out of sample on the regime-test partition,
7. compare predictive performance and the magnitude of Delta_d.

### 5. Segmentation decision

A pair is labelled SEGMENTED only if both conditions hold:
...
Exact calibration belongs to the study design and may be stated in the paper without embedding the full internal calibration layer in the public code.
...
- stability: the absolute persistence shift, abs(Delta_d), exceeds a high-threshold cutoff, so interpretation should be cautious
